...
_CLAUSE_HEAD = r"ให้ยกเลิกความใน|ยกเลิกความใน|ให้ยกเลิก|เพิ่มความต่อไปนี้เป็น"
_CLAUSE_RE = re.compile(
    rf"(?:{_CLAUSE_HEAD})\s*"
    r"((?:วรรค\S+\s*(?:และวรรค\S+\s*)?ของ\s*)?)"
    rf"มาตรา\s*([๐-๙]+(?:/[๐-๙]+)?)\s*({_ART_ORD})?"
    # ⚠️ ท้าย clause ต้อง "หยุดก่อนถึง clause ถัดไป" — เขียน [^“]{0,400} เฉย ๆ ไม่ได้
    #    เพราะจะกลืน clause ถัดไปทั้งอัน แล้วเสียหาย 2 ทางพร้อมกัน:
    #      1) มาตราถัดไปหายจากกราฟ (ม.๒๐ ในฉบับที่ ๓ หายไปทั้งมาตรา)
    #      2) มาตรานี้ไปคว้าชื่อฉบับก่อนหน้าของมาตราถัดไปมาใส่ (ม.๗ เคยได้ ปว.๙๖ ของ ม.๒๐)
    #    (?!...) เช็คทีละตัวอักษร = เดินต่อได้เรื่อย ๆ แต่หยุดทันทีที่เจอหัว clause ใหม่
    #    ต้องเผื่อถึง 400 เพราะบาง clause ไล่ชื่อมาตรายาว (ม.๑๐๕ ทวิ..อัฏฐ) กว่าจะถึงชื่อฉบับ
    rf"((?:(?!{_CLAUSE_HEAD})[^“]){{0,400}})")
# ตัวตัดท้ายอีกชั้น — เผื่อหัว clause ถัดไปเขียนด้วยสำนวนที่ไม่ได้ดักไว้ข้างบน
# (ม.๑๐๖ ในฉบับที่ ๕ ไม่มีฉบับก่อนหน้า แต่เคยไปคว้าของ clause ถัดไปมาใส่)
_CLAUSE_END_RE = re.compile(r"ให้ใช้ความต่อไปนี้แทน|ให้ยกเลิก|เพิ่มความต่อไปนี้เป็น")

# <ฉบับก่อนหน้า> มี 3 รูปแบบ — ต้องรับให้ครบ ไม่งั้นสายขาดกลาง
# ⚠️ ทุกแพตเทิร์นต้องยอมให้มีช่องว่างแทรกกลางชื่อกฎหมาย — PDF ตัดบรรทัดกลางคำ
#    พอ normalize เป็นช่องว่างเดียวจะได้ "ประมวลกฎหมาย ที่ดิน" / "แก้ไขเพิ่มเติม ประมวล..."
#    เขียนติดกันตายตัวแล้วจะพลาด (สายของ ม.๘๑ กับ ม.๑๐๕ เคยขาดเพราะเรื่องนี้)
_PB_NAME = r"แก้ไขเพิ่มเติม\s*ประมวล\s*กฎหมาย\s*ที่ดิน"
_PREV_PB_NO = re.compile(rf"{_PB_NAME}\s*\(\s*ฉบับที่\s*([๐-๙]+)\s*\)\s*พ\.ศ\.\s*([๐-๙]+)")
# ⚠️ ฉบับแรกไม่มีคำว่า "(ฉบับที่ ๑)" ในชื่อ — ถ้าจับแต่แบบมีเลข สายของ ม.๖๙ ทวิ จะขาด
_PREV_PB_Y = re.compile(rf"{_PB_NAME}\s*พ\.ศ\.\s*([๐-๙]+)")
# ⚠️ ห้ามใช้ [^พ] เป็นตัวคั่น (เคยเป็นแบบนั้นแล้วพัง) — ชื่อเดือนไทย ๓ เดือนมีตัว "พ"
#    อยู่ในตัวเอง: กุมภาพันธ์ / พฤษภาคม / พฤศจิกายน  ปว. ที่ลงวันที่เดือนพวกนี้จึงแกะไม่ออก
#    เช่น "ปว. ฉบับที่ ๙๖ ลงวันที่ ๒๙ กุมภาพันธ์ พ.ศ. ๒๕๑๕" -> ตกเป็น "กฎหมายอื่น" สายขาด
#    (กระทบ ๑๐ มาตรา: ๗ ๑๔ ๕๗ ๕๘ ๕๘ ทวิ ๘๑ ๑๐๓ ๑๐๕ ๑๐๖ ๒๗ ตรี ซึ่งต้นสายคือ ปว.๙๖)
#    ใช้ .{0,40}? แบบ lazy แทน — หยุดที่ "พ.ศ." ตัวแรกจริง ๆ ไม่ใช่ตัวอักษร พ ตัวแรก
# ⚠️ อีก ๒ รูปแบบที่เจอในเอกสารจริง: เขียน "ประกาศคณะปฏิวัติ" (ไม่มี "ของ") และใช้คำเต็ม
#    "พุทธศักราช" แทน "พ.ศ." — ฉบับที่ ๑๔ อ้าง ปว.๓๓๔ ด้วยสำนวนนี้ทั้งคู่ สายของ ม.๑๔ เลยขาด
_PREV_REV = re.compile(r"ประกาศ\s*(?:ของ)?\s*คณะปฏิวัติ\s*ฉบับที่\s*([๐-๙]+)"
                       r".{0,40}?(?:พ\.ศ\.|พุทธศักราช)\s*([๐-๙]+)")
...
def _parse_prev(tail: str) -> "dict | None":
    ...
    # ตัดท้ายให้จบแค่ clause ของตัวเอง — ไม่งั้นมาตราที่ไม่มีฉบับก่อนหน้าจะไปคว้าชื่อฉบับ
    # ของ clause ถัดไปมาตอบ (เจอจริงที่ ม.๑๐๖ ในฉบับที่ ๕ ซึ่งแก้ทับตัวบทเดิม ๒๔๙๗)
    cut = _CLAUSE_END_RE.search(tail)
    if cut:
        tail = tail[:cut.end()]
...
# ── ดัชนีตัวละคร (entity index) ────────────────────────────────────────────────
# กราฟการแก้ไขข้างบนตอบได้เรื่องเดียว: "ใครแก้มาตราไหน" มันไม่รู้ว่า "ใครมีอำนาจอะไร"
# คำถามแนว "ผู้มีอำนาจเพิกถอนโฉนด ยังมีอำนาจอื่นใดอีก" จึงตอบไม่ได้ทุกวิธี เพราะเป็น
# คำถามแบบ "รวบรวมให้ครบ" ซึ่งการค้นแบบ top-k ทำไม่ได้โดยธรรมชาติ — วัดจริงแล้ว
# คำว่า "อธิบดี" อยู่ใน 13 มาตรา แต่ระบบดึงได้ทีละ 12 ก้อน (ซ้ำมาตราเดิมด้วย)
# ไล่ hop เพิ่มกี่รอบก็ไม่ครบ เพราะการค้นเรียงตาม "ความคล้าย" ไม่ใช่ "ความครบ"
#
# ทางแก้: ให้โค้ดกวาดตัวบททั้งเล่มตอน index แล้วนับว่า entity ไหนโผล่มาตราใดบ้าง
# ครบ 100% และนิ่ง 100% เพราะเป็นการนับ ไม่ใช่การเดา (หลักเดียวกับ amendment_graph)
#
# ⚠️ ตารางนี้คือส่วนที่ผูกกับโดเมน — ย้ายไปใช้กับเอกสารชุดอื่นให้แก้เฉพาะตรงนี้
#    ตัวอัลกอริทึมข้างล่างไม่รู้จักกฎหมายเลย มันแค่ "นับ pattern ต่อหน่วยเอกสาร"
ENTITY_PATTERNS: dict[str, str] = {
    # ผู้มีอำนาจ — ใช้ (?<!...) / (?!...) กันคำที่ซ้อนกันให้นับแยกกันถูกตัว
    "อธิบดี":              r"อธิบดี",
    "รัฐมนตรี":            r"(?<!คณะ)รัฐมนตรี",      # อย่านับ "คณะรัฐมนตรี" มาเป็นตัวนี้
    "คณะรัฐมนตรี":         r"คณะรัฐมนตรี",
    "คณะกรรมการจัดที่ดินแห่งชาติ": r"คณะกรรมการ(?!สอบสวน)",   # "คณะกรรมการสอบสวน" คนละองค์กร
    "เจ้าพนักงานที่ดิน":   r"เจ้าพนักงานที่ดิน",
    "พนักงานเจ้าหน้าที่":  r"พนักงานเจ้าหน้าที่",
    "ผู้ว่าราชการจังหวัด": r"ผู้ว่าราชการจังหวัด",
    "นายอำเภอ":            r"นายอำเภอ",
    # เอกสารสิทธิ — คำถามมักอ้างถึงเอกสารโดยไม่รู้เลขมาตรา
    "โฉนดที่ดิน":          r"โฉนดที่ดิน",
    "ใบจอง":               r"ใบจอง",
    "หนังสือรับรองการทำประโยชน์": r"หนังสือรับรองการทำประโยชน์",
    "ใบไต่สวน":            r"ใบไต่สวน",
    "ใบแทน":               r"ใบแทน",
    "หนังสือสำคัญสำหรับที่หลวง": r"หนังสือสำคัญสำหรับที่หลวง",
}
_ENTITY_RE = {k: re.compile(v) for k, v in ENTITY_PATTERNS.items()}
# แยกว่าตัวไหนเป็น "ผู้กระทำ" ตัวไหนเป็น "ของ" — คำถามแนวอำนาจ/หน้าที่ต้องเล็งผู้กระทำ
# ไม่งั้นจะไปหยิบชื่อเอกสารที่บังเอิญอยู่ในคำถามมาแทน ("ผู้มีอำนาจเพิกถอน*โฉนดที่ดิน*")
ENTITY_ACTORS = {"อธิบดี", "รัฐมนตรี", "คณะรัฐมนตรี", "คณะกรรมการจัดที่ดินแห่งชาติ",
                 "เจ้าพนักงานที่ดิน", "พนักงานเจ้าหน้าที่", "ผู้ว่าราชการจังหวัด", "นายอำเภอ"}
_entity_cache: "dict | None" = None


def in_force_doc() -> str:
    """path ของ 'ฉบับใช้บังคับปัจจุบัน' (Update-vlast) — ตัวบทที่ใช้จริง ณ วันนี้"""
    _ensure_loaded()
    c = next((c for c in _chunks if c.get("group") == GROUP_IN_FORCE
              and not c.get("is_scan")), None)
    return os.path.join(DATA_DIR, c["source"]) if c else ""


def entity_index() -> dict[str, list[str]]:
    """{ชื่อ entity: [เลขมาตราที่กล่าวถึง]} คำนวณจากตัวบทที่ใช้บังคับปัจจุบัน

    ตอบคำถามที่การค้นตอบไม่ได้: 'X ถูกกล่าวถึงในมาตราใดบ้าง (ทั้งหมด)'
    ผลเป็นการนับล้วน ๆ จึงครบเสมอและรันซ้ำได้ผลเดิมทุกครั้ง"""
    global _entity_cache
    if _entity_cache is not None:
        return _entity_cache
    path = in_force_doc()
    arts = doc_articles(path) if path else {}
    out: dict[str, list[str]] = {}
    for name, rx in _ENTITY_RE.items():
        hits = [a for a, body in arts.items() if rx.search(body)]
        if hits:
            out[name] = hits
    _entity_cache = out
    return out
...
def question_entities(question: str) -> list[str]:
    """entity ที่ข้อความนี้พาดพิงถึง — เรียงจาก 'ชื่อยาว/เจาะจงกว่า' ไปหาสั้นกว่า
    เพื่อไม่ให้ 'หนังสือรับรองการทำประโยชน์' ถูกกลืนโดยตัวที่กว้างกว่า
    (ใช้กับตัวบทก็ได้ ไม่ได้จำกัดแค่คำถาม)"""
    q = question or ""
    found = [n for n, rx in _ENTITY_RE.items() if rx.search(q)]
    return sorted(found, key=lambda n: -len(n))


def count_entities(text: str) -> dict[str, int]:
    """นับ 'จำนวนครั้ง' ที่แต่ละ entity ถูกเอ่ยถึง — ใช้ชี้ว่าใครเป็นประธานของข้อความนี้

    นับครั้งไม่ใช่แค่เจอ/ไม่เจอ เพราะมาตราเดียวเอ่ยถึงหลายตัว แต่ตัวที่เป็นประธาน
    จะถูกเอ่ยซ้ำ ๆ (ม.๖๑ พูดว่า "อธิบดี" 8 ครั้ง ส่วนตัวอื่นครั้งสองครั้ง)"""
    return {n: len(rx.findall(text or "")) for n, rx in _ENTITY_RE.items()
            if rx.search(text or "")}


@traced("TOOL")
def entity_brief(name: str, max_arts: int = 20) -> str:
    """รายชื่อมาตราทั้งหมดที่กล่าวถึง entity นี้ — เอาไปวางหัว context ให้ LLM เห็นว่ามีอะไรบ้าง

    ไม่ได้บอกว่า 'ความสัมพันธ์คืออะไร' (นั่นต้องอ่านตัวบทเอา) แต่บอกว่า 'ต้องไปดูที่ไหนบ้าง'
    ซึ่งเป็นข้อมูลที่การค้นให้ไม่ได้ เพราะการค้นคืนมาแค่ก้อนที่คล้ายคำถามที่สุด"""
    arts = entity_index().get(name, [])
    if not arts:
        return ""
    shown = arts[:max_arts]
    more = f" (แสดง {len(shown)} จาก {len(arts)})" if len(arts) > len(shown) else ""
    return (f'มาตราทั้งหมดที่กล่าวถึง "{name}" ในฉบับใช้บังคับปัจจุบัน '
            f"— รวม {len(arts)} มาตรา{more}:\n"
            f"  {', '.join('มาตรา ' + a for a in shown)}\n"
            f"  [รายการนี้โค้ดนับจากตัวบททั้งเล่ม จึงครบถ้วน — "
            f"ถ้าจะตอบว่า \"มีเท่านี้\" ให้ยึดรายการนี้]")


@traced("TOOL")
def entity_articles(name: str, max_arts: int = 14, max_chars: int = 900) -> str:
    """ตัวบทของทุกมาตราที่กล่าวถึง entity นี้ — ดึงตรงจากไฟล์ ไม่ผ่านการค้น

    ⚠️ ห้ามไปหยิบจาก _chunks: ก้อนสำหรับค้นถูกหั่นตามความยาว ไม่ได้หั่นตามมาตรา
       มาตราสั้น ๆ หลายมาตราจึงถูกยัดรวมอยู่ในก้อนเดียว แล้วก้อนนั้นติดป้ายเป็นมาตราแรก
       ผลคือดึงตามเลขมาตราแล้วได้ศูนย์ก้อน (ม.๕๑ ๕๔ ๙๑ ๙๘ เจอปัญหานี้) หรือได้เศษ
       กลางมาตราที่ไม่มีประโยคหลัก (ม.๒๗ ได้ท่อน "(๓) วิธีที่ผู้เข้าครอบครอง...")
       ดึงจาก doc_articles() แทน — เป็นแหล่งเดียวกับที่ใช้สร้างดัชนี จึงตรงกันเสมอ

    ตัดที่ max_chars เพราะประโยคที่ให้อำนาจอยู่ต้นมาตราเสมอ ("ให้อธิบดีมีอำนาจ...")
    ท่อนหลังเป็นรายละเอียดวิธีปฏิบัติ ซึ่งไม่จำเป็นกับคำถามแนว 'มีอำนาจอะไรบ้าง'"""
    arts = entity_index().get(name, [])
    if not arts:
        return ""
    bodies = doc_articles(in_force_doc())
    out = [f'ตัวบทของทุกมาตราที่กล่าวถึง "{name}" (รวม {len(arts)} มาตรา '
           f"— โค้ดกวาดจากตัวบททั้งเล่ม จึงครบถ้วน):"]
    for a in arts[:max_arts]:
        body = bodies.get(a, "")
        if not body:
            continue
        clipped = body[:max_chars] + (" ..." if len(body) > max_chars else "")
        out.append(f"\n[มาตรา {a}] {clipped}")
    return "\n".join(out)
